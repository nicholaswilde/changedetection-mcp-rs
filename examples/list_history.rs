...
async fn main() {
    ...
    let watches = client.list_watches(None).await.expect("Failed to list watches");
    
    for (uuid, watch) in watches {
        let history = client.get_watch_history(&uuid).await.expect("Failed to get history");
        println!("Watch {}: {} - history count: {}", uuid, watch.url, history.len());
    }
}
